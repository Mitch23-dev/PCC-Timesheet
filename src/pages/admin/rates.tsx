...
export default function RatesPage() {
  ...
  const [schemaLoadDetail, setSchemaLoadDetail] = useState<string | null>(null);
  ...
  async function loadAll() {
    if (!adminPw) return;
    setLoading(true); setError(null); setSchemaLoadDetail(null); setStatus("Loading rates…");
    try {
      ...
      setSchemaLoadDetail(j?.schemaReady === false && j?.schemaLoadDetail ? String(j.schemaLoadDetail) : null);
    } catch (e: any) {
    ...
    <main className="admin-shell">
      ...
      <div className="tabcard">
        ...
        <section className="card tabcard-body resources-shell">
          ...
          {(status || error || schemaLoadDetail) && (
            <div className={error || schemaLoadDetail ? "alert alert-bad" : "subtle"} style={{ marginTop: 10 }} role={error || schemaLoadDetail ? "alert" : undefined}>
              {(error || status) && <div style={{ fontWeight: 700 }}>{error || status}</div>}
              {schemaLoadDetail && (
                <div style={{ marginTop: error || status ? 8 : 0, fontFamily: "ui-monospace, monospace", fontSize: 12, whiteSpace: "pre-wrap", wordBreak: "break-word", opacity: 0.95 }}>
                  {schemaLoadDetail}
                </div>
              )}
            </div>
