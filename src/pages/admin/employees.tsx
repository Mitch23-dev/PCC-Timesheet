    ...
    try {
      ...
      const r = await fetch("/api/admin/employees/create", {
        ...
        body: JSON.stringify({
          ...
          notes: newEmp.notes,
          timesheet_type: newEmp.timesheet_type,
        }),
      });
      const data = await r.json().catch(() => ({}));
      if (!r.ok) {
  ...
  return (
    <>
      ...
      <div className="admin-shell">
        ...
        <PageHeader
          title="Employees"
          subtitle="Click an employee to view / edit full details."
          actions={
            <>
              <button onClick={() => loadEmployees()} disabled={loading} className="btn btn-ghost btn-compact">Refresh</button>
              <button onClick={() => setShowAdd((v) => !v)} className="btn btn-ghost btn-compact">{showAdd ? "Close" : "Add Employee"}</button>
              <button onClick={signOut} className="btn btn-ghost btn-compact">Sign out</button>
            </>
          }
        />
        ...
        {showAdd ? (
          <div className="admin-form-card" style={{ marginTop: 16 }}>
            <SectionHeader title="New Employee" />
            ...
            <FormSection className="ui-form-grid-2">
            ...
            <div className="ui-form-grid-3">
              ...
              <div>
                <div style={{ fontSize: 12, opacity: 0.8 }}>Timesheet Type</div>
                <div className="subtle" style={{ marginBottom: 6 }}>Choose which timesheet layout this employee will use.</div>
                <ScrollableDropdown
                  value={newEmp.timesheet_type}
                  options={[
                    { value: "standard", label: "Standard" },
                    { value: "management", label: "Management Grid" },
                    { value: "mechanic", label: "Mechanic Grid" },
                  ]}
                  onChange={(next) => setNewEmp((p) => ({ ...p, timesheet_type: next }))}
                />
              </div>
              <div>
        ...
        <div className="admin-table-card ui-card-pad" style={{ marginTop: 16, padding: 4 }}>
          <DataTable>
