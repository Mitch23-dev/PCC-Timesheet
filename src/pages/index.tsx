...
import { EMPLOYEES, JOB_TYPES } from "@/lib/constants";
import { MaterialCatalogSource, buildMaterialLabel, getMaterialOptionsForSource, normalizeMaterialCatalog } from "@/lib/materialCatalog";
...
import WeeklyGridTimesheet from "@/components/WeeklyGridTimesheet";
...
export default function Home() {
  ...
  const [sessionEmployee, setSessionEmployee] = useState<{ id: string; name: string; timesheet_type?: string | null } | null>(null);
  ...
  const [materialCatalog, setMaterialCatalog] = useState<MaterialCatalogSource[]>([]);
  const [materialCatalogReady, setMaterialCatalogReady] = useState(false);
  const [equipmentCatalog, setEquipmentCatalog] = useState<EquipmentCatalogItem[]>([]);

  const [materialRows, setMaterialRows] = useState<MaterialRow[]>([
    { source: "", material: "", otherMaterial: "", loads: "", notes: "" },
  ]);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const j = await r.json();
        if (r.ok) setMaterialCatalog(normalizeMaterialCatalog(Array.isArray(j?.sources) ? j.sources : []));
      } catch {
        // ignore
      } finally {
        setMaterialCatalogReady(true);
      }
    })();
  }, []);
...
// Convert HEIC/HEIF -> JPEG in-browser so uploads "just work".
    ...
    formData.append("materialRows", JSON.stringify(materialRows.map((row) => ({
      ...row,
      material: row.material ? buildMaterialLabel(row.source, row.material) || row.material : "",
      source_name: row.source || null,
      material_name: row.material || null,
    }))));
    ...
    setMaterialRows([{ source: "", material: "", otherMaterial: "", loads: "", notes: "" }]);
    setPhotos(null);
  }
...
<div>
  ...
  if (sessionEmployee?.timesheet_type === "management" || sessionEmployee?.timesheet_type === "mechanic") {
    return (
      <WeeklyGridTimesheet
        employee={sessionEmployee}
        type={sessionEmployee.timesheet_type as any}
        logout={logout}
      />
    );
  }
...
<h1 className="h1" style={{ margin: 0 }}>Timesheet</h1>
      ...
      <div className="tabcard">
        <EmployeeTabs active="entry" timesheetType={sessionEmployee?.timesheet_type} />
      ...
      <section className="card employee-panel-card ui-card-pad">
        <SectionHeader
          title="Equipment"
          actions={<button className="btn btn-ghost employee-secondary-btn" onClick={addEquipRow}>+ Add Equipment</button>}
        />

        {equipRows.map((row, idx) => {
          const isDumpTruck = row.equipment === "Dump Truck";
          const attachmentOptions = getAttachmentOptionsForEquipment(equipmentCatalog, row.equipment);
          ...
          return (
            <div key={idx} style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #eee" }}>
                  ...
                  <ScrollableDropdown
                    ...
                    onChange={(equipment) => {
                      setEquipRows((prev) => {
                        ...
                        const nextAttachmentOptions = getAttachmentOptionsForEquipment(equipmentCatalog, equipment);
                        const nextAttachment = nextAttachmentOptions.includes(copy[idx].attachment || "")
                          ? (copy[idx].attachment || "")
                          : "";

                        copy[idx] = { ...copy[idx], equipment, attachment: nextAttachment };
                ...
                {showAttachment ? (
                  <label>
                    ...
                    <ScrollableDropdown
                      ...
                      options={getAttachmentDropdownOptionsForEquipment(equipmentCatalog, row.equipment)}
                      placeholder="None"
                      onChange={(next) =>
                        setEquipRows((prev) => {
                          const copy = [...prev];
              ...
              <div className="ui-grid-two" style={{ marginTop: 10 }}>
                <label>
                ...
                <button
                  className="btn employee-inline-btn"
      ...
      <section className="card employee-panel-card ui-card-pad">
        <SectionHeader
          ...
          actions={<button className="btn btn-ghost employee-secondary-btn" onClick={addMaterialRow}>+ Add Material</button>}
        />

        {materialRows.map((row, idx) => {
          const materialOptions = getMaterialOptionsForSource(materialCatalog, row.source);
          return (
            <div key={idx} style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #eee" }}>
              <div className="material-grid material-grid-3">
                <label>
                  ...
                  <ScrollableDropdown
                    value={row.source || ""}
                    options={materialCatalog.filter((source) => source.is_active !== false).map((source) => source.name)}
                    placeholder={materialCatalogReady ? "Select source" : "Loading sources…"}
                    onChange={(source) =>
                      setMaterialRows((prev) => {
                        const copy = [...prev];
                        copy[idx] = { ...copy[idx], source, material: "", otherMaterial: "" };
                        return copy;
                      })
                    }
                  />
                ...
                <label>
                  ...
                  <ScrollableDropdown
                    ...
                    placeholder={row.source ? (materialOptions.length ? "Select material" : "No materials") : "Select source first"}
                    onChange={(material) =>
                      setMaterialRows((prev) => {
                        const copy = [...prev];
      ...
      <section className="card employee-panel-card ui-card-pad">
        <SectionHeader title="Slip Photos (optional)" />
        <div className="muted" style={{ marginBottom: 8 }}>
          Tip: On phones you can choose Camera or Photo Library.
        </div>
        <div className="row" style={{ justifyContent: "space-between" }}>
          <label className="btn btn-ghost employee-secondary-btn" style={{ alignSelf: "flex-start" }}>
            + Add Slip Photos
            <input
              type="file"
              accept="image/*"
