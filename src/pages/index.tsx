...
export default function Home() {
  ...
  const [equipRows, setEquipRows] = useState<EquipRow[]>([
    { equipment: "Komatsu 138 (Old)", attachment: "None", hours: "", notes: "", truckingHours: "", truckingNotes: "" },
  ]);

  const [materialRows, setMaterialRows] = useState<MaterialRow[]>([
    { material: `Conrads - 3/4" Clear Stone`, otherMaterial: "", loads: "", notes: "" },
  ]);
...
// Convert HEIC/HEIF -> JPEG in-browser so uploads "just work".
    ...
    setTotalHours("");
    setHeaderNotes("");
    setEquipRows([{ equipment: "Komatsu 138 (Old)", attachment: "None", hours: "", notes: "", truckingHours: "", truckingNotes: "" }]);
    setMaterialRows([{ material: `Conrads - 3/4" Clear Stone`, otherMaterial: "", loads: "", notes: "" }]);
    setPhotos(null);
  }
    ...
    <main className="page">
      <div className="topbar">
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {sessionEmployee && <Link className="btn btn-ghost" href="/my">My Timesheets</Link>}
        </div>
...
<h1 className="h1" style={{ margin: 0 }}>Timesheet</h1>
        ...
        <div className="ts-grid">
          <label>
            ...
            {sessionEmployee ? (
              ...
              <select className="input" value={employee} onChange={(e) => setEmployee(e.target.value as any)}>
          ...
          <label>
            ...
            <select className="input" value={jobType} onChange={(e) => setJobType(e.target.value as any)}>
          ...
          <label style={{ gridColumn: "1 / -1" }}>
            ...
            <input className="input" value={headerNotes} onChange={(e) => setHeaderNotes(e.target.value)} />
          </label>
        </div>
      </section>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setEquipRows((prev) => {
                        ...
                        if (!EXCAVATORS.has(equipment) && equipment !== "Dump Truck" && !isSkid) copy[idx].attachment = "None";
                        if ((equipment === "Dump Truck" || isSkid) && !copy[idx].attachment) copy[idx].attachment = "None";
                        return copy;
                      });
                    }}
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e) => setEquipRows((prev) => {
                        const copy = [...prev];
                        copy[idx] = { ...copy[idx], attachment: e.target.value };
                        return copy;
                      })}
                      style={{ width: "100%", padding: 10 }}
                    >
                      {(isDumpTruck ? DUMP_TRUCK_ATTACHMENTS : isSkidSteer ? SKID_STEER_ATTACHMENTS : ATTACHMENTS).map((a) => (
      ...
      <button onClick={handleSubmit} style={{ width: "100%", padding: 16, fontSize: 18, fontWeight: 900 }}>
        Submit
      </button>
