...
export default function MyTimesheets() {
  ...
  const [employee, setEmployee] = useState<{ id: string; name: string; timesheet_type?: string | null } | null>(null);
  const [weeklyRows, setWeeklyRows] = useState<any[] | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (sj?.employee?.timesheet_type === "management" || sj?.employee?.timesheet_type === "mechanic") {
          await loadWeekly(sj.employee.timesheet_type);
        } else {
          await Promise.all([load(), loadMaterialOptions(), loadEquipmentOptions()]);
        }
      } catch {
        router.replace("/");
      }
  ...
  async function loadWeekly(type: string) {
    setErr(null);
    const r = await fetch(`/api/me/weekly/list?type=${encodeURIComponent(type)}`);
    const j = await r.json();
    if (!r.ok) {
      setErr(j?.error || "Failed to load weekly timesheets");
      return;
    }
  ...
  async function loadMaterialOptions() {
    try {
      const r = await fetch("/api/material-catalog");
      const j = await r.json();
      if (r.ok) setMaterialOptions(flattenMaterialCatalog(normalizeMaterialCatalog(Array.isArray(j?.sources) ? j.sources : [])));
    } catch {
      // ignore
    }
  }
  ...
  function addEquip() {
    const firstEquipment = equipmentOptions[0] || "";
    const firstAttachment = null;
    setEditEquip((p) => [
      ...p,
      {
  ...
  function addMat() {
    setEditMat((p) => [...p, { material: materialOptions[0] || "", loads: 0, notes: null }]);
  }
  ...
  async function saveEdit() {
    ...
    const payload = {
      ...
      equipment: editEquip.map((e) => {
        ...
        return {
          equipment: e.equipment,
          attachment: attachmentOptions.length ? (e.attachment || null) : null,
  ...
  if (employee?.timesheet_type === "management" || employee?.timesheet_type === "mechanic") {
    const totalWeekly = (weeklyRows || []).reduce((sum, row) => sum + Number(row.total_hours || 0), 0);
    return (
      <main className="page my-shell">
        <div className="tabcard">
          <EmployeeTabs active="my" timesheetType={employee?.timesheet_type} />
          <section className="card tabcard-body employee-panel-card">
            <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button className="btn btn-ghost employee-secondary-btn" onClick={async () => { try { await fetch('/api/me/logout', { method: 'POST' }); } catch {} router.replace('/'); }}>Sign out</button>
            </div>
            <div className="card" style={{ marginTop: 14, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 18 }}>Weekly Grid Hours (All Time)</div>
                  <div className="subtle">{employee.timesheet_type === "mechanic" ? "Mechanic" : "Management"} entries</div>
                </div>
                <div style={{ fontWeight: 900, fontSize: 28 }}>{totalWeekly.toFixed(1)}</div>
              </div>
            </div>
          </section>
        </div>
        {err && <div className="alert alert-bad" style={{ marginBottom: 14 }}>{err}</div>}
        <section className="card employee-panel-card ui-card-pad">
          <SectionHeader
            title="Weekly History"
            actions={<button className="btn btn-ghost employee-secondary-btn" onClick={() => loadWeekly(employee.timesheet_type || "management")}>Refresh</button>}
          />
          {!weeklyRows && <div className="subtle" style={{ marginTop: 12 }}>Loading…</div>}
          {weeklyRows && weeklyRows.length === 0 && <div className="subtle" style={{ marginTop: 12 }}>No weekly timesheets yet.</div>}
          {(weeklyRows || []).map((row) => (
            <div key={row.id} style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #eee" }}>
              <div className="row" style={{ justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
                <div>
                  <div style={{ fontWeight: 800 }}>{fmtWeek(row.week_start)}</div>
                  <div className="subtle">{row.status === "submitted" ? "Submitted" : "Draft"} • {row.entries?.length || 0} entries</div>
                </div>
                <div style={{ fontWeight: 800 }}>{Number(row.total_hours || 0).toFixed(1)} hrs</div>
              </div>
              <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                {(row.entries || []).slice(0, 6).map((entry: any, idx: number) => (
                  <div key={idx} className="card" style={{ padding: 10 }}>
                    <div style={{ fontWeight: 700 }}>{entry.entry_date} • {entry.start_time} - {entry.end_time} • {Number(entry.hours || 0).toFixed(1)} hrs</div>
                    <div>{entry.job_label || entry.equipment_label}</div>
                    {entry.attachment_label ? <div className="subtle">Attachment: {entry.attachment_label}</div> : null}
                    <div className="subtle">{entry.description}</div>
                  </div>
                ))}
                {(row.entries || []).length > 6 && <div className="subtle">+ {(row.entries || []).length - 6} more entries</div>}
              </div>
            </div>
          ))}
        </section>
  ...
  return (
    <main className="page my-shell">
      ...
      <section className="card employee-panel-card ui-card-pad">
        <SectionHeader
          title="Weekly Summary (Thu → Wed)"
          actions={<button className="btn btn-ghost employee-secondary-btn" onClick={load}>Refresh</button>}
        />
                  ...
                  return (
                    <div key={r.id} style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", padding: "10px 12px", border: "1px solid #eee", borderRadius: 12 }}>
                      ...
                      <div className="my-entry-actions">
                        ...
                        <button className="btn btn-ghost employee-secondary-btn" onClick={() => openEdit(r.id)}>View</button>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <ModalSection>
                  <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button className="btn btn-ghost" onClick={addEquip}>+ Add Equipment</button>
                  </div>
                  {editEquip.map((e, idx) => {
                    const isDump = e.equipment === "Dump Truck";
                    const attachmentOptions = getAttachmentOptionsForEquipment(equipmentCatalog, e.equipment);
                    ...
                    return (
                      <div key={idx} style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #f0f0f0" }}>
                        <div className="equip-grid">
                          <label>
                            ...
                            <ScrollableDropdown
                              ...
                              onChange={(next) => setEditEquip((p) => {
                                ...
                                copy[idx] = { ...copy[idx], equipment: next, attachment: nextAttachmentOptions.length ? (nextAttachmentOptions.includes(copy[idx].attachment || "") ? (copy[idx].attachment || "") : "") : null };
                                return copy;
                              })}
                            />
                          </label>
                          ...
                          {showAttachment ? (
                            <label>
                              ...
                              <ScrollableDropdown
                                value={e.attachment || ""}
                                options={getAttachmentDropdownOptionsForEquipment(equipmentCatalog, e.equipment)}
                                placeholder="None"
                                onChange={(next) => setEditEquip((p) => {
                                  const copy = [...p];
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 120px", gap: 10, marginTop: 10, alignItems: "end" }}>
                          ...
                          <button className="btn admin-inline-btn" onClick={() => setEditEquip((p) => p.filter((_, i) => i !== idx))}>Remove</button>
                        </div>
                      </div>
                    );
                  })}
                ...
                <ModalSection>
                  <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button className="btn btn-ghost" onClick={addMat}>+ Add Material</button>
                  </div>
                  {editMat.map((m, idx) => (
                    <div key={idx} className="materials-item" style={{ marginTop: 10 }}>
                      <div className="materials-top">
                        ...
                        <button
                          className="btn admin-inline-btn materials-remove"
                ...
                <ModalSection>
                  <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <label className="btn btn-ghost" style={{ cursor: slipUploading ? "not-allowed" : "pointer", opacity: slipUploading ? 0.7 : 1 }}>
                  ...
                  <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                    ...
                    {editPhotos.map((p: Photo) => (
                      <div key={p.path} className="card admin-soft-card" style={{ padding: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
