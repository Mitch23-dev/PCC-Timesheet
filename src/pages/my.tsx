  ...
  return (
    <main className="page">
      <div className="topbar">
        <Link className="btn btn-ghost" href="/">← Timesheet Entry</Link>
      </div>
                ...
                .map((r) => (
                  <div key={r.id} style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", padding: "10px 12px", border: "1px solid #eee", borderRadius: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <div style={{ fontWeight: 900, whiteSpace: "nowrap" }}>{Number(r.total_hours || 0).toFixed(1)}h</div>
              ...
              <div style={{ marginTop: 12 }}>
                <div className="ts-grid">
                  ...
                  <label>
                    ...
                    <select className="input" value={editData.job_type} onChange={(e) => setEditData((p: any) => ({ ...p, job_type: e.target.value }))}>
                  ...
                  <label style={{ gridColumn: "1 / -1" }}>
                    ...
                    <textarea className="input" style={{ minHeight: 70 }} value={editData.job_text_clean || ""} onChange={(e) => setEditData((p: any) => ({ ...p, job_text_clean: e.target.value }))} />
                  </label>
                  <label style={{ gridColumn: "1 / -1" }}>
                    <div className="ts-label">Notes</div>
                    <input className="input" value={editData.notes || ""} onChange={(e) => setEditData((p: any) => ({ ...p, notes: e.target.value }))} />
                  </label>
                </div>
                ...
                <div style={{ marginTop: 12, borderTop: "1px solid #eee", paddingTop: 12 }}>
                  <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button className="btn btn-primary" onClick={addEquip}>+ Add</button>
                  </div>
                  {editEquip.map((e, idx) => {
                    const isDump = e.equipment === "Dump Truck";
                    const isSkid = e.equipment === "Kubota Skid Steer" || e.equipment === "John Deere Skid Steer";
                      ...
                      <div key={idx} style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #f0f0f0" }}>
                        <div className="equip-grid">
                          <label>
                            ...
                            <select value={e.equipment} onChange={(ev) => setEditEquip((p) => {
                              const c = [...p];
                              c[idx] = { ...c[idx], equipment: ev.target.value };
                              return c;
                          ...
                          {(isExc || isDump || isSkid) ? (
                            <label>
                              ...
                              <select value={e.attachment || "None"} onChange={(ev) => setEditEquip((p) => {
                                const c = [...p];
                                c[idx] = { ...c[idx], attachment: ev.target.value };
                                return c;
                              })} style={{ width: "100%", padding: 10 }}>
                                {(isDump ? DUMP_TRUCK_ATTACHMENTS : isSkid ? SKID_STEER_ATTACHMENTS : ATTACHMENTS).map((a) => <option key={a} value={a}>{a}</option>)}
                              </select>
                            </label>
                          ) : <div />}
                          ...
                          {!isDump ? (
                            <label className="equip-hours">
                              ...
                              <input inputMode="decimal" value={String(e.hours ?? "")} onChange={(ev) => setEditEquip((p) => {
                                const c = [...p];
                            ...
                            <label>
                              ...
                              <input inputMode="decimal" value={String(e.trucking_hours ?? "")} onChange={(ev) => setEditEquip((p) => {
                                const c = [...p];
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 120px", gap: 10, marginTop: 10 }}>
                          <label>
                            ...
                            <input value={isDump ? (e.trucking_notes || "") : (e.notes || "")} onChange={(ev) => setEditEquip((p) => {
                              const c = [...p];
                          ...
                          <button className="btn btn-ghost" onClick={() => setEditEquip((p) => p.filter((_, i) => i !== idx))}>Remove</button>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div style={{ marginTop: 12, borderTop: "1px solid #eee", paddingTop: 12 }}>
                  <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button className="btn btn-primary" onClick={addMat}>+ Add</button>
                  </div>
                  {editMat.map((m, idx) => (
                    <div key={idx} style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1fr 120px 1fr 120px", gap: 10, alignItems: "end" }}>
                      <label>
                        <div style={{ fontWeight: 700, marginBottom: 6 }}>Material</div>
                        <select value={m.material} onChange={(ev) => setEditMat((p) => {
                          const c = [...p];
                          c[idx] = { ...c[idx], material: ev.target.value };
                          return c;
                        })} style={{ width: "100%", padding: 10 }}>
                      ...
                      <label>
                        ...
                        <input inputMode="decimal" value={String(m.loads ?? 0)} onChange={(ev) => setEditMat((p) => {
                          const c = [...p];
                      ...
                      <label>
                        ...
                        <input value={m.notes || ""} onChange={(ev) => setEditMat((p) => {
                          const c = [...p];
                          c[idx] = { ...c[idx], notes: ev.target.value };
                          return c;
                        })} style={{ width: "100%", padding: 10 }} />
                      </label>
                      <button className="btn btn-ghost" onClick={() => setEditMat((p) => p.filter((_, i) => i !== idx))}>Remove</button>
                    </div>
                  ))}
                </div>
