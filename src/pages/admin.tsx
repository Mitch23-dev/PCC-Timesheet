...
// Keep options aligned with the worker entry UI.
const JOB_TYPES = [
  "Civil",
  "Concrete",
  "Asphalt",
          ...
          <div className="card tabcard-body" style={{ padding: 16, marginBottom: 14 }}>
            ...
            <div className="adminFilters" style={{ marginTop: 12 }}>
              ...
              <div className="adminFiltersBtn">
                <button className="btn" onClick={clearDates} disabled={loading}>
                  Clear Dates
                </button>
              </div>
              ...
              <div className="field">
                ...
                <select className="input" value={jobSummary} onChange={(e) => setJobSummary(e.target.value)}>
            ...
            <div className="adminActions">
              ...
              <button className="btn" onClick={exportBatchPDF} disabled={!adminPw || loading}>
                Export Batch PDF
              </button>
              <button className="btn" onClick={() => setLockRange(true)} disabled={!adminPw || loading}>
                Lock Range
              </button>
              <button className="btn" onClick={() => setLockRange(false)} disabled={!adminPw || loading}>
                Unlock Range
              </button>
                ...
                <div style={{ marginTop: 12 }}>
                  <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                    ...
                    <div>
                      ...
                      <select className="input" value={editTimesheet.job_type} onChange={(e) => setEditTimesheet((p: any) => ({ ...p, job_type: e.target.value }))}>
                        {JOB_TYPES.map((t) => (
                    ...
                    <div style={{ width: "100%" }}>
                      ...
                      <textarea className="input" style={{ minHeight: 70 }} value={editTimesheet.job_text_clean || ""} onChange={(e) => setEditTimesheet((p: any) => ({ ...p, job_text_clean: e.target.value }))} />
                    </div>
                    <div style={{ width: "100%" }}>
                      <div className="label">Notes</div>
                      <input className="input" value={editTimesheet.notes || ""} onChange={(e) => setEditTimesheet((p: any) => ({ ...p, notes: e.target.value }))} />
                    </div>
                  </div>
                          ...
                          <div key={idx} style={{ border: "1px solid rgba(255,255,255,0.10)", borderRadius: 12, padding: 10 }}>
                            <div className="row" style={{ gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                              <div style={{ minWidth: 220 }}>
                                ...
                                <select className="input" value={e.equipment} onChange={(ev) => setEditEquip((p) => p.map((x, i) => i === idx ? ({ ...x, equipment: ev.target.value }) : x))}>
                              ...
                              {!isDump && (
                                <div style={{ minWidth: 200 }}>
                                  ...
                                  <select className="input" value={e.attachment || "None"} onChange={(ev) => setEditEquip((p) => p.map((x, i) => i === idx ? ({ ...x, attachment: ev.target.value }) : x))}>
                                    {ATTACHMENT_OPTIONS.map((o) => <option key={o} value={o}>{o}</option>)}
                                  </select>
                                </div>
                              )}
                              ...
                              <button className="btn" onClick={() => setEditEquip((p) => p.filter((_, i) => i !== idx))}>Remove</button>
                            </div>

                            {!isDump && (
                              <div style={{ marginTop: 8 }}>
                                <div className="label">Notes</div>
                                <input className="input" value={e.notes || ""} onChange={(ev) => setEditEquip((p) => p.map((x, i) => i === idx ? ({ ...x, notes: ev.target.value }) : x))} />
                              </div>
                            )}
                            ...
                            {isDump && (
                              <div style={{ marginTop: 8 }}>
                                ...
                                <input className="input" value={e.trucking_notes || ""} onChange={(ev) => setEditEquip((p) => p.map((x, i) => i === idx ? ({ ...x, trucking_notes: ev.target.value }) : x))} />
                              </div>
                            )}
                          </div>
                        );
                  ...
                  <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid rgba(255,255,255,0.12)" }}>
                    <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <button className="btn" onClick={addMat}>+ Add</button>
                    </div>
                    <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
                      {editMat.map((m, idx) => (
                        <div key={idx} style={{ border: "1px solid rgba(255,255,255,0.10)", borderRadius: 12, padding: 10 }}>
                          <div className="row" style={{ gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                            <div style={{ minWidth: 260 }}>
                              ...
                              <select className="input" value={m.material} onChange={(ev) => setEditMat((p) => p.map((x, i) => i === idx ? ({ ...x, material: ev.target.value }) : x))}>
                            ...
                            <button className="btn" onClick={() => setEditMat((p) => p.filter((_, i) => i !== idx))}>Remove</button>
                          </div>
                          <div style={{ marginTop: 8 }}>
                            <div className="label">Notes</div>
                            <input className="input" value={m.notes || ""} onChange={(ev) => setEditMat((p) => p.map((x, i) => i === idx ? ({ ...x, notes: ev.target.value }) : x))} />
                          </div>
                        </div>
                      ))}
                    </div>
