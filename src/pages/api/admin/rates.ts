...
function formatSupabaseLoadError(err: any) {
  if (!err) return "";
  const parts = [err.message, err.hint, err.details].filter((p: any) => typeof p === "string" && p.trim());
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (firstError) {
      if (isMissingSchemaError(firstError)) {
        return res.status(200).json({
          ...
          schemaReady: false,
          error:
            "Rates could not be loaded (database schema mismatch). Run supabase/sql_rates_api_prerequisites.sql in the Supabase SQL editor, or apply migrations v21+ and v26.",
          schemaLoadDetail: formatSupabaseLoadError(firstError),
        });
      }
  ...
  for (const row of equipmentClasses) {
    ...
    if (Number.isFinite(idNum) && idNum > 0) {
      const { error } = await supabaseServer.from("equipment_classes").upsert({ ...payload, id: idNum }, { onConflict: "id" });
      if (error) return res.status(500).json({ error: error.message || "Failed to save equipment class" });
    } else {
      // No DB id yet (or temp client id stripped): insert would fail if migration already seeded this name — upsert on unique name.
      const { error } = await supabaseServer.from("equipment_classes").upsert(payload, { onConflict: "name" });
      if (error) return res.status(500).json({ error: error.message || "Failed to save equipment class" });
    }
  }
  for (const row of attachments) {
    ...
    if (Number.isFinite(attId) && attId > 0) {
      const { error } = await supabaseServer.from("attachments").upsert({ ...payload, id: attId }, { onConflict: "id" });
      if (error) return res.status(500).json({ error: error.message || "Failed to save attachment" });
    } else {
      const { error } = await supabaseServer.from("attachments").upsert(payload, { onConflict: "equipment_class_id,name" });
      if (error) return res.status(500).json({ error: error.message || "Failed to save attachment" });
    }
  }
