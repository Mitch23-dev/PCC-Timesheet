...
type EmployeeTab = "entry" | "my" | "equipment";

export default function EmployeeTabs({ active, timesheetType }: { active: EmployeeTab; timesheetType?: string | null }) {
  return (
    <div className="tabbar">
      ...
      {timesheetType === "mechanic" ? (
        <a href="/equipment-reference" className={active === "equipment" ? "tab tab-active" : "tab"}>
          <span className="tabIcon" aria-hidden="true">
            <svg width="18" height="18" viewBox="0 0 24 24">
              <path d="M4 7h16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
              <path d="M7 7v10m5-10v10m5-10v10" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
              <path d="M3 17h18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            </svg>
          </span>
          <span className="tabLabel">Equipment</span>
        </a>
      ) : null}    </div>
  );
}
